...
do
        ...
        if [ -f "${fq}_sorted_nodup_poo_samples.MASKED.bed" ]; then
            echo "Skipping ${fq}_sorted_nodup_poo_samples.MASKED.bed as it already exists."
        else
            ...
            cmd="minimap2 -t 16 -ax map-ont \
                    ...
                    | bamToBed > ${fq}_sorted_nodup_poo_samples.MASKED.bed"
            echo ${cmd}
            eval ${cmd}
        fi
done
